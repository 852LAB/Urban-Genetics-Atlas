...
function marketRefreshExposureTheme(){
    ...
    if(['Market Exposure','Transaction Exposure','Transaction Pulse'].includes(select?.value)){
...
async function marketFetch(){
    ...
    for(const url of urls){
        try{
            ...
            if(data.analytics?.transaction_activity?.version!==MARKET_TRANSACTION_VERSION) throw new Error(`market transaction mismatch: ${data.analytics?.transaction_activity?.version||'missing'}`);
...
function marketTransactionForDistrict(district){
    const activity=marketContext?.analytics?.transaction_activity;
    if(!activity || !district)return null;
    const cleaned=marketCleanDistrict(district);
    const sourceGeography=activity.had_to_source?.[cleaned];
    if(!sourceGeography)return null;
    const a=activity.geographies?.[sourceGeography];
    if(!a)return null;
    return {source_geography:sourceGeography,source_label:sourceGeography, ...a};
}
function marketTransactionExposureFor(transaction,exposure){
    const pulse=Number(transaction?.transaction_pulse);
    const local=Number(exposure?.local_opportunity);
    if(!Number.isFinite(pulse)||!Number.isFinite(local))return null;
    return marketClamp(pulse*local);
}
function marketFormatMoneyMillions(v){
    const n=Number(v); if(!Number.isFinite(n))return '—';
    if(Math.abs(n)>=1000)return `HK$${(n/1000).toFixed(n>=10000?1:2)}bn`;
    return `HK$${Math.round(n).toLocaleString()}m`;
}
function marketTransactionSparkline(series,trend){
    const rows=(series||[]).filter(d=>Number.isFinite(Number(d.transactions)));
    if(rows.length<2)return '';
    const vals=rows.map(d=>Number(d.transactions)); let lo=Math.min(...vals),hi=Math.max(...vals); if(hi===lo){lo-=1;hi+=1;}
    ...
    return `<div class="market-spark market-transaction-spark ${marketTrendClass(trend)}"><div class="market-spark-head"><span>ASP building-unit transactions</span><small>${marketEsc(start)} → ${marketEsc(end)}</small></div><svg viewBox="0 0 100 28" preserveAspectRatio="none" aria-hidden="true"><line x1="2" y1="25" x2="98" y2="25"></line><polyline points="${points}"></polyline></svg></div>`;
}
function marketTransactionBlock(ctx){
    const t=ctx.transaction; if(!t)return '';
    return `<div class="market-group market-transaction-group"><div class="market-group-title">Transaction Activity</div><div class="market-transaction-head"><div><span>Transaction Pulse</span><strong>${marketScore(t.transaction_pulse)}</strong><small>${marketEsc(t.pulse_label||'—')}</small></div><div><span>Transaction Exposure</span><strong>${marketScore(ctx.transactionExposure)}</strong><small>Pulse × local opportunity</small></div><div><span>12-month direction</span><strong class="${marketTrendClass(t.trend_12m_pct)}">${marketTrendArrow(t.trend_12m_pct)} ${marketEsc(marketFormatTrend(t.trend_12m_pct))}</strong><small>3-month average vs year ago</small></div></div><div class="market-trend-cards"><div><span>Latest ASP count</span><strong>${Number(t.latest_transactions||0).toLocaleString()}</strong></div><div><span>Latest consideration</span><strong>${marketEsc(marketFormatMoneyMillions(t.latest_consideration_m_hkd))}</strong></div><div><span>Recent 3m average</span><strong>${Number(t.recent_3m_avg||0).toLocaleString(undefined,{maximumFractionDigits:0})}</strong></div><div><span>Vs recent norm</span><strong>${Number.isFinite(Number(t.activity_ratio))?(Number(t.activity_ratio)*100).toFixed(0)+'%':'—'}</strong></div></div>${marketTransactionSparkline(t.series,t.trend_12m_pct)}<div class="market-transaction-source">Source geography: ${marketEsc(t.source_geography||'—')} · Land Registry ASP building-unit registrations.</div></div>`;
}
...
function marketObsFor(feature){
    ...
    return {hexId,district:district||null,region,analytics,exposure,transaction,transactionExposure:marketTransactionExposureFor(transaction,exposure),
...
// === MARKET TRANSACTION V1 TEST MARKER ===
